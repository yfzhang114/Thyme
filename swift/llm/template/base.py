...
class Template(ProcessorMixin):
    special_tokens = ['<image>', '<video>', '<audio>', '<bbox>', '<ref-object>', '<cot-process>', '<start-image>', '<cot-process>', '<processed_image>', '</processed_image>', '<sandbox_output>', '</sandbox_output>']
    ...
    def encode(self,
        ...
        if isinstance(inputs, dict):
            ...
            if not self.is_training and not self.O3:
    ...
    #         elif context in ['<processed_image>', '</processed_image>', '<sandbox_output>', '</sandbox_output>']:
    #             loss_scale = 0.
    #             c_list = [context]
    #         else:
    #             c_list = [context]
            
    #         # Check if the current context is sandwiched between <sandbox_output> and </sandbox_output>
    #         is_sandwiched = (
    #             i > 0 and i < len(context_list) - 1 and
    #             context_list[i-1] == '<sandbox_output>' and
    #             context_list[i+1] == '</sandbox_output>'
    #         )

    #         if is_sandwiched:
    #             # If sandwiched, the loss for elements derived from this context item is 0.
    #             loss_scale = 0.
    ...
    def _pre_tokenize_images(self, context_list: List[Context], loss_scale_list: List[float],
        ...
        target_idx = -1
        if context_list.count('</sandbox_output>') >= 2:
            try:
                rev_list = context_list[::-1]
                # Find first occurrence in reversed list (which is last in original)
                first_idx_in_rev = rev_list.index('</sandbox_output>')
                # Find second occurrence in reversed list (which is second to last in original)
                second_idx_in_rev = rev_list.index('</sandbox_output>', first_idx_in_rev + 1)
                # Convert back to original index
                target_idx = len(context_list) - 1 - second_idx_in_rev
            except ValueError:
                # This should not happen because of the count check
                pass
