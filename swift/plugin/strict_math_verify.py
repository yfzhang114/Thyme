...
from swift.plugin import math_normalize
from swift.plugin import math_equal
...
def is_multiple_choice(content):
    # Check if the question is a multiple-choice question
    if "Choices:\\n" in content or "\\nChoices:" in content or "\nChoices:" in content:  # If it contains "Choices:\n", it is a multiple-choice question
        return True
    if "\\nA" in content and "\\nB" in content:  # If it contains both \nA and \nB, it is a multiple-choice question
        return True
    if "\nA" in content and "\nB" in content:  # If it contains both \nA and \nB, it is a multiple-choice question
        return True
    if "A:" in content and "B:" in content:  # If it contains both \nA and \nB, it is a multiple-choice question
        return True
    return False

def extract_option(answer):
    # This regular expression ignores all parentheses, brackets, and spaces, and extracts the letter A-E
    match = re.match(r"^[^\w]*([A-E])", answer.strip())  # Match and ignore non-alphabetic characters (e.g., parentheses)
    return match.group(1) if match else None

def verify_choice_answer(content, answer, ground_truth):
    # First, check if it is a multiple-choice question
    if is_multiple_choice(content):
        # If it is a multiple-choice question, extract the student's and the correct answer's letter
        option = extract_option(answer)
        correct_option = extract_option(ground_truth)

        # Compare the extracted letters
        if option == correct_option:
            return True
    return False  # If not a multiple-choice question or the answers don't match, return 0


if __name__ == "__main__":
    evaluate_math("the answer is x > 3", 'x < 3')